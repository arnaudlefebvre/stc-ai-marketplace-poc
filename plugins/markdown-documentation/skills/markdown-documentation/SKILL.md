...
#### Mandatory Mermaid Validation

Whenever a Markdown document contains one or more `mermaid` fenced code
blocks, validate every block with `@aj-archipelago/merval` before delivering
the document. Do not rely only on visual inspection or balanced fences.

`merval` is a JavaScript library, not a CLI. Import and call its API:

```javascript
import { validateMermaid } from '@aj-archipelago/merval';

const result = validateMermaid(mermaidSource);
if (!result.isValid) {
  throw new Error(JSON.stringify(result.errors));
}
```

- Extract and validate each Mermaid block independently so one parser failure
  cannot hide later diagrams.
- Report the file, block number or source line, diagram type, and validation
  errors when a block is invalid.
- Correct invalid diagrams and rerun validation until every changed or created
  Mermaid block passes.
- When updating a versioned document, preserve the existing version and create
  the next version if the user's documentation rules require it.
- State the validator package version and the number of diagrams validated in
  the final result.
- If `merval` is unavailable, do not claim Mermaid syntax was validated. Explain
  the limitation and request installation or authorization to use it.
